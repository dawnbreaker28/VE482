 ...
 */
  ...
  if (!rdy_head[q]) {		/* add to empty queue */
      ...
      rp->p_readysize = 1;     /* init queue size */
  } 
  else {					/* add to tail of queue */
      rdy_tail[q]->p_nextready = rp;		/* chain tail of queue */	
      rdy_tail[q] = rp;				/* set new queue tail */
      rp->p_nextready = NULL;		/* mark new end */
      ++rdy_head[q]->p_readysize;      /* increment queue size */
  }
...
static void enqueue_head(struct proc *rp)
{
  ...
  if (!rdy_head[q]) {		/* add to empty queue */
      ...
      rp->p_readysize = rdy_head[q]->p_readysize + 1;     /* move and increment queue size */
 ...
 */
  ...
  rdy_head_ptr = get_cpu_var_ptr(rp->p_cpu, run_q_head[q]);
  rdy_tail = get_cpu_var(rp->p_cpu, run_q_tail);
   ...
   */
  prev_xp = NULL;				
  for (xpp = rdy_head_ptr; *xpp;
		  xpp = &(*xpp)->p_nextready) {
      if (*xpp == rp) {				/* found process to remove */
          if (xpp == rdy_head_ptr) {        /* queue head removed */
              if(rp->p_nextready) {
                  rp->p_nextready->p_readysize = rp->p_readysize - 1;       /* move and decrement queue size */
              }
          } else {
              --(*rdy_head_ptr)->p_readysize;       /* decrement queue size */
          }
 ...
 */
  ...
  u64_t t;
  u64_t lottery_pool[NR_SCHED_QUEUES];
  u64_t lottery_total;
  u64_t lottery;

  /* Use time as a random number
   * Time of interrupts is used to generate randomness (see kernel/system/do_irqctl.c)
   * so I suppose it is also good enough to use time as random number here.
   */
  read_tsc_64(&t);
  lottery_total = 0;
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {
      if (rp = rdy_head[q]) {
          lottery_pool[q] = rp->p_readysize * (NR_SCHED_QUEUES - q);
          lottery_total += lottery_pool[q]; /* q = 0 is the highest priority */
      } else {
          lottery_pool[q] = 0;
          TRACE(VF_PICKPROC, printf("cpu %d queue %d empty\n", cpuid, q););
      }
  }
  if(lottery_total == 0) { /* No runnable process */
      return NULL;
  }
  lottery = t % lottery_total;

  /* Find the lottery, first the queue then the proc */
  for (q=0; q < NR_SCHED_QUEUES; q++) {
      if (lottery >= lottery_pool[q]) { /* Not in this queue */
          lottery -= lottery_pool[q];
          continue;
      }
      rp = rdy_head[q];
      lottery /= (NR_SCHED_QUEUES - q); /* now lottery corresponds to position index */
      while(lottery > 0) {
          assert(rp);
          rp = rp->p_nextready;
          --lottery;
      }
      assert(rp && proc_is_runnable(rp));
